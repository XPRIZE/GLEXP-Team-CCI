...
function animPlate(mod) {
    ...
    if (mod == 'half' || mod == 'full') {
        ...
        console.log("Log: " + 'Unknown mod of <b>' + mod + '</b> in animPlate function call');
    }
}
        ...
        if (mMode && !book.dialog.open) {
            ...
            if (page) {
                ...
                if (dragRet) {
                    ...
                    if (!curSequence || book.interruptTime == 0) {
            ...
            if (noneHit) {
                ...
                if (linkHit.action == "Open Page First Time" || linkHit.action == "Open Page") {
                    ...
                    if (linkHit.targets) {
                        ...
                        console.log("Log: " + "Open page link has no targets.");
                    }
...
function checkLogicLinks() {
    ...
    for (var l = 0; l < book[curPage - 1].logics.length; l++) {
        ...
        if (comparitor) {
            ...
            console.log("Log: " + "Unknown conditional of <b>" + action + "</b>. For the time, only '=' is supported");
        }
    }
    /*
    if (matchedArr.length > 1) {
        console.log("Number of conditions met are more than one. We haven't talked about this yet");
...
*/
    // NEW SHIT, we are sloppy like Ray. Combining targets from all matched logic links, and making a hybrid garbage thing.
    // FUCK it we'll switch to the new engine and set shit straight.
   
    if (matchedArr.length > 0) {
        let targets = [];
        while (matchedArr.length > 0) {
            let newTargs = matchedArr.shift();
            targets = targets.concat(book[curPage - 1].logics[newTargs].targets);
        }
        if (curSequence && curSequence.isRunning) {
            curSequence.end();
        }
        curSequence = new sequence(targets, book[curPage - 1], "logic", [maxDim[0] / 2, maxDim[1] / 2], "logic");
    ...
    if (page) {
        ...
        if (objectDragging) {
            ...
            if (create) {
                ...
                console.log(dragObj);
                if (dragObj.dragMovesForwards === true) {
                    for (let obj in page.objs) {
                        dragObj.layer = Math.max(page.objs[obj].layer, dragObj.layer);
                    }
                    dragObj.layer++;
