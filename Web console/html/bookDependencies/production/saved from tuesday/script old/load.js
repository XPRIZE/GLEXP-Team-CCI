  ...
  function loginElements() {
    ...
    book.login.toggle = function () {
      ...
      if (bookLoginStat == 'open') {
        ...
        // book login area in animation, function canceled;
      }
    };
  ...
  function info() {
    ...
    if (maxDim[0] == 0 && maxDim[1] == 0) {
      ...
      if (!maxDim[0] || !maxDim[1]) {
        throw new fatalError("Page dimensions are missing in XML. \n\n<SinglePageWidth>" + maxDim[1] + "</SinglePageWidth> \n<SinglePageHeight> " + maxDim[0] + "</SinglePageHeight>");
      }
    }
...
function Graphic(curXmlLnk, triggers, key) {
  ...
  if (splitReturn[0].toLowerCase() == "click") {
    linkArrayName = "clicks";
  } else if (splitReturn[0].toLowerCase() == "drop") {
    linkArrayName = "drops";
  } else if (splitReturn[0].toLowerCase() == "startline") {
    linkArrayName = "lineStarts";
  } else if (splitReturn[0].toLowerCase() == "endline") {
    linkArrayName = "lineEnds";
  } else if (splitReturn[0].toLowerCase() == "open page" || splitReturn[0].toLowerCase() == "open page first time") {
...
function Page(key, curXmlPage) {
  ...
  this.redraw = function () {
    ...
    function getFontSizeAndOffsetFromMaxWidth(text, fontName, maxWidth, maxHeight) {
      var curSize = 1;
      var retSize = 0;
      var maxLoop = 10000;
      var i = 0;
      var whichLimiting = false;
      var leftOver = 0;

      while (!retSize) {
        btx.font = curSize + "pt " + fontName;
        var curWidth = btx.measureText(text).width;
        if (curWidth > maxWidth) {
          retSize = curSize - 1;
          whichLimiting = "width";
          leftOver = maxHeight - retSize;
        } else if (curSize > maxHeight) {
          retSize = curSize - 1;
          whichLimiting = "height";
          leftOver = maxWidth - curWidth;
        } else {
          curSize++;
        }
        i++;
        if (i > maxLoop) {
          retSize = 1;
          console.log("loop maxed out");
        }
      }
      var ret = {};
      ret.size = retSize;
      ret.whichLimiting = whichLimiting;
    ...
    function drawText(curText) {
      ...
      if (curText.display == "points") {
        curText.contents = pageElem.points;
      }
      var fontName = "serif";
      var fontCenterObj = getFontSizeAndOffsetFromMaxWidth(
        curText.contents, fontName, curText.initWidth, curText.initHeight
      );
      btx.font = fontCenterObj.size + "pt " + fontName;
      var textLeft = curText.left;
      var textTop = curText.top + fontCenterObj.size;
      if (fontCenterObj.whichLimiting == "height") {
        textLeft += fontCenterObj.leftOver / 2;
      } else {
        textTop += fontCenterObj.leftOver / 2;
      }
      btx.fillStyle = curText.color;
      try {
        // text,x,y,maxWidth
        btx.fillText(curText.contents, textLeft, textTop);
      }
      catch (e) {
      }
    }
