...
let Sq_Players_Child = {
    ...
    animation: function () {
        ...
        this.obj = this._Pubbly.findObj(this.target.chosenDestination);
    ...
    audio: function () {
        ...
        this.elem = this._Pubbly.buffer.assets[this.target.chosenDestination].elem;
    ...
    flash: function () {
        ...
        this.play_child = function () {
            this.flashInt = window.setInterval(function () {
                ...
                _This._Pubbly.drawPage(); // defaults to curPage
    ...
    sequence: function () {
        ...
        this.drawInt = false;
        this.loopsLeft = this.target.loops * 1;
        this.obj = _This._Pubbly.findObj(this.target.chosenDestination);
        this.obj.at = 0;
...
function Sequence(pubblyScope) {
    ...
    this.runTarget = function (target) {
        ...
        switch (target.type) {
            case "drawing tool":
                let cursorSrc = _Pubbly.presets[target.chosenDestination + "-cursor"].src;
                _Pubbly.drawingTool = {
                    type: target.chosenDestination,
                    color: target.value,
                    width: target.width,
                    src: cursorSrc,
                    cursorCSS: "url('" + cursorSrc + "'), auto",
                }
                // For correct hover cursors in events.js
                _Pubbly.events.cursorKey.draw = _Pubbly.drawingTool.cursorCSS;
                break;
            case "animation":
            ...
            case "audio":
                ...
                break;
            case "flash":
                obj = _Pubbly.findObj(target.chosenDestination);
            ...
            case "propertyChange":
                ...
                let things = [];
                let findFuncName = false;

                if (target.destination == "all links") {
                    // Every link
                    for (let linkName in _Pubbly.data.pages[_Pubbly.curPage].linkKeys) {
                        things.push(linkName);
                    }
                } else if (target.destination == "all objects") {
                    // Every object
                    for (let objName in _Pubbly.data.pages[_Pubbly.curPage].objKeys) {
                        things.push(objName);
                    }
                } else {
                    ...
                    if (typeof target.destination.pop == "undefined") {
                        if (target.chosenDestination !== target.destination) {
                            ...
                            things.push(target.chosenDestination);
                        } else {
                            // Vanilla single thing destination
                            things.push(target.destination);
                        }
                    } else {
                ...
                if (target.destinationType == "link") {
                    findFuncName = "findLink";
                } else if (target.destinationType == "object") {
                    findFuncName = "findObj";
                } else {
                    error("log", "sequence", "Unknown object or property to set: " + JSON.stringify(target));
                }

                for (let t = 0; t < things.length; t++) {
                    let thing = _Pubbly[findFuncName](things[t]);
            ...
            case "reset":
                ...
                if (resetType == "object") {
                    searchStart = _Pubbly.findObj(target.destination);
                } else if (resetType == "link") {
                    searchStart = _Pubbly.findLink(target.destination);
                } else if (resetType == "page") {
                    searchStart = _Pubbly.data.pages[_Pubbly.curPage];
                }
