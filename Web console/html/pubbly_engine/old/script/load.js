...
$(document).ready(function () {
    ...
    if (typeof (xml) === "undefined") {
        window.hashLink = window.location.href.split("#");
        if (hashLink.length > 1) {
            window.hashLink = window.hashLink[1];
            xmlLoc = window.assetsLoc + window.hashLink + ".xml";
        } else if (!window.xmlLoc) {
...
function xmlLoaded(curXML) {
    ...
    window.setTimeout(function () {
        book.fonts.add("cac_champagne", "cac_champagne");
    }, 1000);
    ...
    function elements2() {
        ...
        book.textEntryHidden = addElem(bookC, "textarea", [["id", "textEntryHidden"], ["type", "text"], ["autocorrect", "off"], ["autocomplete", "off"], ["autocapitalize", "off"], ["spellcheck", "off"]]);
        book.curField = {};
    ...
    function info() {
        ...
        let longName = getValue(curInfo, "PrjNameLong");
        book.subID = getValue(curInfo, "subID", true);
        if (book.subID) {
            // longName += "#" + window.hashLink;
        }
        document.getElementById("textAct").innerHTML = longName;
        ...
        if (book.subID) {
            book.assetLoc = {
                "img": book.subID + "_images/",
                "aud": book.subID + "_audio/",
                "vid": book.subID + "_videos/"
            }
        } else {
            book.assetLoc = {
                "img": "images/",
        ...
        if (pDisplay == 'BlockSpread') {
            ...
            if (last_bg_left != '' && last_bg_right != '') {
                ...
                book.lastPageDouble = false;
                let infoDouble = getValue(curInfo, "LastPageSingleOrDouble", false)
                if (infoDouble == "Double") {
                    book.lastPageDouble = true;
                } else if (!infoDouble) {
                    if (pageNumberingStr[pageNumberingStr.length - 1].split("-").length > 1) {
                        book.lastPageDouble = true;
                        console.log("Log: " + "Temporary fix for lastPageDouble, check load.js line " + thisLineNumber() + " for info.");
    ...
    let origAmbientAudioSrc = getValue(curInfo, "AmbientAudio", true) || false;
    book.ambientAudio = new AmbientAudio(origAmbientAudioSrc); // false if no orig source
        ...
        for (lnkKey = 0; lnkKey < xmlLnkArr.length; lnkKey++) {
            ...
            for (var t = 0; t < tmpTriggers.length; t++) {
                ...
                if (linkType == 'graphic' || linkType == 'conditional' || linkType == 'page') {
                    ...
                    console.log("Log: " + 'unknown link type <b>' + linkType + '</b>');
                }
            }
...
function Dialog(curXmlObj, key) {
    ...
    $(this.elem).css({
        'background-image': 'url(' + book.assetLoc.img + this.background + ')',
...
function Graphic(curXmlLnk, triggers, key) {
    ...
    if (splitReturn[0] == "click") {
        ...
        console.log("Log: " + "ERROR - unknown TriggerType (page " + key + ")  -- " + splitReturn[0]);
    }
...
function Page(key, curXmlPage) {
        ...
        for (var obj = 0; obj < this.objKey.length; obj++) {
            ...
            if (curObj.type == 'video') {
                ...
                if (curObj.cloneID) {
                    ...
                    if (curObj.extension == "gif") {
                        curObj.currentFrame = 0;
                        if (curObj.isPlaying && !curObj.normallyPlaying) {
                            curObj.isPlaying = false;
                        }
                    }
                    if (curObj.type == "field") {
                        curObj.contents = curObj.origContents;
                    }
                    curObj.rot = 0; // No init rot.
                    curObj.vis = curObj.initVis;
    ...
    this.redraw = function () {
        ...
        for (button in this.buttons) {
            ...
            btx.font = font.size + 'pt Arial'; // buttons have no font
        ...
        function drawText(curText) {
            ...
            if (curText.vis == "show") {
                ...
                if (curText.size !== "auto") {
                    ...
                    var paragraphs = contents.toString().split(/\r\n|\r|\n/g);
                    ...
                    for (var p = 0; p < paragraphs.length; p++) {
                        ...
                        var clickPt = book.curField.insertionClick;
                    ...
                    if (curText.editable && book.curField.name == curText.name && book.curField.insertionPointAlpha && y - lineHeight < maxY) {
                        if (clickPt) {
                            ...
                            var curLine = 0;
                            var absChar = 0;
                            for (y = 0; y < linesOfText.length; y++) {
                                if (curText.top + (y * curText.size) < clickPt.y) {
                                    curLine = y;
                                    absChar += linesOfText[y].length;
                                }
                            }
                            ...
                            var curChar = 0;
                            if (curText.left + metrics.width - (prevChar.width / 2) <= clickPt.x) {
                                prevChar = 0;
                            } else {
                                while (curText.left + metrics.width - (prevChar.width / 2) > clickPt.x) {
                                    prevChar = btx.measureText(line.charAt(line.length - 1));
                                    line = line.substring(0, line.length - 1);
                                    metrics = btx.measureText(line);
                                }
                            }
                            ...
                            absChar += curChar;
                            setCaretToPos(document.getElementById("textEntryHidden"), absChar);
                        }

                        // Important variables are linesOfText, lineHeight, curText.top
                        var yMod = 0; // 1 if click on second line, 2 if on third, ect
                        var xMod = ""; // "ASDF" if click on line that has "ASDF".
                        var insertionAt = $('#textEntryHidden').prop("selectionStart");
                        var insertionLoc = {};
                    ...
                    if (curText.size == "auto") {
                        ...
                        for (var r = 0; r < rows.length; r++) {
                            var row = rows[r];
                            btx.font = row.size + "px " + curText.fontName;
        ...
        if (this.DUPCAN) {
            this.DUPCAN.width = this.DUPCAN.width;
    ...
    if (typeof badImages != 'undefined' && badImages[this.name]) {
            ...
            for (k = 0; k < animObjs.length; k++) {
                ...
                if (xmlAnimData != "") {
                    ...
                    if (curAnim.data[0]) {
                        ...
                        console.log("Log: " + "Problem with animation data on object <b>" + this.name + "</b>. Check console for details");
        ...
        if (this.type == 'image') {
            ...
            if ((this.extension && this.extension.toLowerCase() == "gif" && (gifInfo[this.fileName] && parseInt(gifInfo[this.fileName].length))) || this.isSequence) {
                this.src = book.assetLoc.img + "gifFrames/" + this.fileName + "_frame_0." + this.extension;
                this.currentFrame = 0;
                this.frames = [];
                if (this.isSequence) {
                ...
                if (tmpFileName == "Keypad") {
                    ...
                    this.src = book.assetLoc.img + tmpFileName + "." + this.extension;
                }
            }
            ...
            this.origContents = this.contents;
...
let AmbientAudio = function (src) {
    ...
    this.aud = new Audio();
    this.aud.volume = 0;
    this.aud.onerror = function (err) {
        if (_This.oldSrc) {
            _This.src = _This.oldSrc;
            _This.oldSrc = false;
            _This.aud.src = _This.src;
            //  _This.play(); // Called fom onload?
        } else {
            console.error("Ambient audio could not load any sources... will not play");
            if (_This.change.running && _This.change.finishedCB) {
                _This.change.finishedCB(); // incase everything errored, sequence must go on.
            }
        }
    }
    this.loaded = false;
    ...
    this.aud.addEventListener("canplaythrough", function () {
        _This.start();
        if (_This.change.running) {
            _This.change.int = window.setInterval(function () {
                _This.change.fadeIn.call(_This);
            }, 2);
        }
    });
    this.start = function () {
    ...
    this.loop = function () {
        if (_This.aud.error === null) {
            _This.aud.currentTime = 0;
            _This.aud.play();
    ...
    this.changeSrc = function (src) {
        ...
        let justFile = src.split("audio").length > 1 ? src.split("audio/")[1] : src;
        this.src = book.assetLoc.aud + justFile;
        if ([".mp3", ".wav", ".ogg"].indexOf(this.src.substring(this.src.length - 4)) == -1) {
            // guess?
            this.src = this.src = ".wav";
        }
    ...
    this.fadeAct = function () {
        if (_This.fade.dir == 1 && _This.aud.volume + _This.fade.modVal >= _This.fade.to
            ...
            _This.aud.volume += _This.fade.modVal;
        }
    }

    this.change = {
        fadeOut: function () {
            if (this.aud.volume > 0.01) {
                this.aud.volume -= 0.01;
            } else {
                this.changeSrc(this.change.newSrc);
                this.change.dir = 1;
                window.clearInterval(this.change.int);
            }
        },
        fadeIn: function () {
            if (this.aud.volume < this.change.oldVol) {
                this.aud.volume += 0.01;
            } else {
                this.change.dir = 0;
                window.clearInterval(this.change.int);
                if (this.change.finishedCB) {
                    this.change.finishedCB(); // next targ if we wait for the change
                }
                this.change.running = false;
            }
        }
    }
    this.callTarget = function (target, cb) {
        target[0] = target[0].toLowerCase();
        target[1] = target[1].toLowerCase();
        if (target[0] == "ambient sound") {
            if (target[1] == "fade") {
                window.clearInterval(this.fade.int);
                this.fade = {
                    to: false,
                    in: false,
                    modVal: false,
                ...
                this.fade.to = target[2].split(" ")[0] / 100;
                this.fade.to = Math.max(Math.min(this.fade.to, 1), 0); // Ensure it's between 0 and 1
                let speedFuzzy = target[2].split(" ")[1].toLowerCase();
                // Instant:0, fast:1, med:2, slow:3
                this.fade.in = ["instant", "fast", "medium", "slow"].indexOf(speedFuzzy);
                if (this.fade.in == -1) {
                    this.fade.in = 1000; // default;
                    console.warn("Unknown fade speed, defaulting to 1s");
                } else {
                    this.fade.in *= 1000; // fast:1000, med:2000, slow:3000
                }
                let intSpeed = 10; // for easy mod.
                ...
                cb();
            } else if (target[1] == "change") {
                window.clearInterval(this.change.int);
                // window.clearInterval(this.fade.int);

                this.isPlaying = true;
                this.change.running = true;
                this.change.oldVol = this.aud.volume * 1;
                this.change.newSrc = target[2];
                this.change.dir = -1;
                this.change.finishedCB = cb;
                this.change.int = window.setInterval(function () {
                    _This.change.fadeOut.call(_This);
                }, 2);
            } else {
                console.error("Unknown ambient type of " + target[1]);
            }
        } else {
            console.error("Target not recognized as an ambient sound target... ignoring");
        }
    }
    ...
    let testing = false; // shows the thing
    if (testing) {
        document.body.appendChild(this.aud);
        this.aud.controls = true;
        this.aud.style.position = "absolute";
        this.aud.style.top = "0px";
        this.aud.style.left = "0px";
    }
}
