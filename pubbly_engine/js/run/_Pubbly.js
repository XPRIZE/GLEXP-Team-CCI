/*
Main entrance function for the pubbly RUN process

Takes a pubbly JSON structured data object, and loads stuff and runs stuff and displays stuff... this is the MAIN for the whole kitten and cabootle.
*/
class Pubbly {
    // Tester function, for "WHERE IS IT"
    drawLinks(ctx, which, color) {
        ...
        let curPage = this.data.pages[this.curPage];
        if (!ctx) {
            ctx = $("canvas.current")[0].getContext("2d");
        }
        which = (which) ? which : Array.apply(null, { length: curPage.links.length }).map(Number.call, Number);
        let links = curPage.links;
        // Converge to always an array
        let drawLinks = (typeof which === "number") ? [which] : which;
        for (let g = 0; g < drawLinks.length; g++) {
            let link = links[drawLinks[g]];
            let poly = (link.type === "graphic") ?
                link.poly :
                imgToPoly(curPage.objs.find(o => o.name === link.name));
            drawLinkAct(poly, ctx, color);
        }
    }
    ...
    checkLocFor(loc, what, condition) {
        ...
        let found = [];
        let curPage = this.data.pages[this.curPage];
        for (let l = 0; l < curPage.links.length; l++) {
            let link = curPage.links[l];
            let poly = false;
            if (link.type === "graphic" && link.enabled) {
                poly = link.poly;
            } else if (link.type === "image") {
                let img = curPage.objs.find(o => o.name === link.name);
                if (img.vis) {
                    poly = imgToPoly(img);
                }
            }
            if (link.pinned) {
                let obj = curPage.objs.find(o => o.name === link.pinned);
                if (obj) {
        ...
        if (checkDrag || checkDraw || checkEdit) {
            for (let o = 0; o < curPage.objs.length; o++) {
                ...
                if (
                    (checkDrag && obj.vis && (obj.mobility === "clone" || obj.mobility === "drag")) ||
        ...
        if (searchStart) {
            found = searchStart.filter(o => {
                if (typeof what === "object") {
                     ...
                     * links aren't .type link. They're graphical links or image links. Objects look like a hardcoded return also
                     * Shouldnt even need this check, because the searchStart differentiates
                     *
                     return (o.name === what &&
    ...
    loadBufferLead(newPage, cbs) {
        ...
        if (missingForDraw === 0) {
            ...
            this.pageBuffer.loadMultiplePages(neededForLead, {
                    ...
                    console.log("Background-buffer done");
                }
            });
        } else {
            // Buffer in foreground
            let progFnc = this.progressGraph.calculate;
            if (!this.firstDraw) {
                this.slowNetworkGif.show();
                this.events.disable();
                this.navigationUI.disable();
                progFnc = this.slowNetworkGif.calculate;
            }
            this.pageBuffer.loadMultiplePages(neededForLead, {
                done: function () {
                    this.slowNetworkGif.hide();
                    this.events.enable();
    ...
    changeCurPage_dispatch(newPage, cbs) {
        ...
        if (typeof newPage === "undefined") {
            ...
            this.loadBufferLead(newPage, {
                    ...
                    // (drawPage doesn't go through unless PUBBLY is ready
                    this.ready = true;
    ...
    draw_readyAndReturnCTX(drawPage, relativeHalf = "") {
        ...
        drawPage = (typeof drawPage === "number") ? drawPage : this.curPage;
    ...
    draw_general(ctx, curObj, curPage) {
        ...
        let relPath = (curObj.type == "sequence") ?
            ...
            curObj.relPath;
        let assetPath = this.pageBuffer.assetListLoaders[curPage].keys[relPath];
        if (curObj.type === "workspace") {
            img = curObj.workspace.elem;
        } else {
            if (assetPath && assetPath.elem) {
                img = assetPath.elem;
            } else {
                img = false;
            }
    ...
    draw_field(ctx, curObj, relPage) {
        if (curObj.vis) {
            ...
            if (text !== "" || curObj.editing) {
                ...
                // At line 0, we are at word 0. After drawing line 0 (which had 5 words in it) we're at word 5... So on
                let lineStartWord = 0;
                for (let l = 0; l < lines.length; l++) {
                    ...
                    words = words.filter(wrd => wrd !== "");
                    if (typeof curObj.highlight.wordNum === "number" &&
                        (curObj.highlight.wordNum >= lineStartWord
                            && curObj.highlight.wordNum < lineStartWord + words.length)) {
                        // The targeted word that needs the highlighting is inside current line
                        let wordOffsetWidth = 0;
                        for (let w = 0; w < words.length; w++) {
                            if (lineStartWord + w === curObj.highlight.wordNum) {
                                if (curObj.highlight.type === "front") {
                                    // FRONT light, meaning color of text itself
                                    ctx.fillStyle = curObj.highlight.color;
                                } else {
                                    // BACK light, meaning box behind word.
                                    ctx.fillStyle = curObj.highlight.color;
                                    let wordWidth = ctx.measureText(words[w]).width;
                                    let spaceWidth = ctx.measureText(" ").width;
                                    ctx.fillRect(
                                        drawLeft + wordOffsetWidth - (spaceWidth * 0.5),
                                        drawTop - curObj.calculated.lineHeight,
                                        wordWidth + spaceWidth,
                                        curObj.calculated.lineHeight * 1.2,
                                    );
                                    ctx.fillStyle = curObj.color
                                }
                            } else {
                            ...
                            ctx.fillText(
                                words[w] + " ",
                                drawLeft + wordOffsetWidth,
                                drawTop
                            );
                            wordOffsetWidth += ctx.measureText(words[w] + " ").width;
                        }
                    } else {
                        ctx.fillText(
                            lines[l],
    ...
    draw_texture(ctx, curTexture, rect) {
        let image = this.presetAssets.list.find(i => i.relPath === "pubbly_engine/assets/textures/" + curTexture);
        if (image && image.elem) {
    ...
    draw_video(ctx, curVideo, curPage) {
        let elem = this.pageBuffer.assetListLoaders[curPage].byFileSource[curVideo.fSrc].elem;
    ...
    constructor(data, runtimeProps) {
        ...
        this.runtimeProps = Object.assign(
            {
                ...
                forceBack: false,
                // Where engine is located, relatively
                enginePathPrefix: "",
            }, runtimeProps);
        /*
        ...
        this.ready = false;
        // Haven't ever drawn a page yet (used to choose between the slowNetWork connection progress graph vs the regular pubbly letters first time progress graph)
        this.firstDraw = true;
        ...
        this.progressGraph = new ProgressGraph("vertical_letters", this.dom.main.find("#loaderCont"), this.runtimeProps.enginePathPrefix);

        this.slowNetworkGif = new ProgressGraph("carrot_eat", this.dom.main.find("#loaderCont"), this.runtimeProps.enginePathPrefix);
        this.slowNetworkGif.hide();

        this.presetAssets = new PubblyPresetAssets(this.runtimeProps.rootToEngine);
        this.pageBuffer = new PubblyPageBuffer(this.data);
        this.domInteractionCover = new DomInteractionCover(this.dom.canvases);
        this.domInteractionCover.picked.relPath = this.runtimeProps.rootToEngine + this.domInteractionCover.picked.relPath;
