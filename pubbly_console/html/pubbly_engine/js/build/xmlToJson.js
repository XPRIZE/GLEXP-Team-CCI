/*
    xmlDoc (javascript xml object) sent in, a bunch of nasty magic happens, JSON object called back
*/
function updateJsonFromXML(xmlDoc, cbs) {
    cbs.prog.say("XML document loaded.");
    let xml = {};
    xml.info = {};
    xml.pages = [];
    ...
    let objSets = {
        ...
        "field": [
            ...
            ["highlightColor", "HighlightColor", "90,172,197", rgb2hex],
            ["highlightStyle", "HighlightStyle", "front", translate({"front":"front", "back":"back"}, "back")],
    ...
    let linkSet = [
        ["name", "Name"],
        ["type", "LinkType", "graphic", translate({"graphic":"graphic", "image":"image"},"graphic")],
            ...
            for (let o = 0; o < objNodes.length; o++) {
                ...
                if (objSets[type]) {
                    ...
                    if (curObj.type == "field") {
                        ...
                        curObj.calculated = {};
                        curObj.highlight = {
                            wordNum: false, // 0 means highlight word index 0, not don't highlight
                            style: curObj.highlightStyle,
                            color: curObj.highlightColor,
                        };
                        delete curObj.highlightStyle;
                        delete curObj.highlightColor
                    }
                    ...
                    if (curLink.triggers[type] !== "undefined") {
                        ...
                        for (let tg = 0; tg < targets.length; tg++) {
                            ...
                            for (let t = 0; t < targetArr.length; t++) {
                                ...
                                }   else if (target.type == "spokenfield") {
                                    if (typeof target.destination == "object" && target.destination.length > 1) {
                                        target.destination.map(function(audName) {
                                            curPage.auds.push({
                                                ext: false, 
                                                fSrc: false,
                                                loaded: false,
    ...
    if (typeof xml.info.lastPageSpread === "undefined") {
    ...
    cbs.prog.say("XML parsed. Checking assets");
    cbs.done(xml);
}
