...
class AssetListLoader {
    loadList(assets, cbs) {
        if (this.inLoop) {
            this.killBack = function () {
                this.killBack = false;
                this.load(assets, cbs);
            }.bind(this);
        } else {
            this.load(assets, cbs);
        }
    ...
    load(assets, cbs) {
        ...
        if (!assets) {
            console.error("No asset to load");
        }
        ...
        if (typeof assets.length == "undefined") {
            ...
            let checkList = function (asset) {
                ...
                // Call back function for outside kill switch, to avoid dup loading
                if (this.killBack) {
                    this.killBack();
                } else if (found == total) {
                    this.inList = false;
                    ...
                    if ((found + missing) == total) {
                        ...
                        cbs.fail("Missing assets in list", missing);
                    } else if (cbs.prog) {
            ...
            let nextAsset = function () {
                ...
                window.setTimeout(function () {
                    // If asset is already loaded... which shouldn't even be the case..
                    // It COULD be the case if a page WAS half buffered.
                    if (asset.elem) {
                        found++;
                        checkList(asset);
                    } else {
    ...
    loadSingleSpecificOrVagueAsset(asset, cbs) {
        ...
        if (typeof asset.type !== "undefined" &&
            ...
            // ../dog is false
            asset.ext = getExtFromRelPath(asset.relPath);
            if (asset.ext === false) {
                // ["wav", "ogg", "mp3"]
                ...
                console.error("AssetListLoader.loadSingleSpecificOrVagueAsset: Asset ext '" + asset.ext + "' not supported. Please only use " + supportedExts[asset.type].join(",") + " files for " + asset.type + " files.");
                cbs.fail();
            }
        } else {
    ...
    download(asset, cbs) {
        ...
        if (asset.type === "image" || asset.type === "gif") {
            ...
            asset.elem.volume = 0;
            // this.hiddenDiv.appendChild(asset.elem);
            loadEvent = "canplaythrough";
        } else {
            console.error("Unknown asset type of " + asset.type);
        }
    ...
    searchForVagueAsset(asset, cbs) {
        ...
        if (check) {
            ...
            let tmp = asset.relPath.split('.');
            tmp.pop();
            asset.relPath = tmp.join(".");
            cbs.fail();
        }
    }
