  ...
  if (book.sprite) {
    ...
    if (window.viewing !== "local") { // local viewings, no sprite. Honestly, I'm just tired of looking at the error when testing.
      if (isFirefox) {
        ...
        book.sprite.src = assetsLoc + "audio/sprite.mp3";
      }
    }
  ...
  function bufferElement(type, name, location) {
    ...
    if (type == "image") {
      ...
      if (!document.getElementById(name)) {
        ...
        if (isFirefox) {
          aud.src = assetsLoc + "audio/" + name + ".ogg";
        } else {
          if (window.viewing == "local") {
            aud.src = assetsLoc + "audio/" + name + ".wav";
          } else {
            aud.src = assetsLoc + "audio/" + name + ".mp3";
          }
        }
        aud.id = name;
        aud.errorNum = 0;
