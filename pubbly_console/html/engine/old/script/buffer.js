...
function loadAssets() {
	...
	if (window.viewing !== "local") { // local viewings, no sprite. Honestly, I'm just tired of looking at the error when testing.
	    if (isFirefox) {
		book.sprite.src = assetsLoc + book.assetLoc.aud + "sprite.ogg";
	    } else {
		book.sprite.src = assetsLoc + book.assetLoc.aud + "sprite.wav";
	    }
	}
	book.sprite.waitingForSilence = false;
	book.sprite.timeout = false;

	// Chrome policy change, no autoplays.
	if (isPad && !isCordova || true) {
	...
	if (spriteKey) {
		...
		console.log("Bug: " + "Audio sprite error, using individual files instead (no preloading on tablets). Check console for details.");
	...
	if (type == "image") {
		...
		if ((objLoc.extension == "gif" || objLoc.isSequence) && objLoc.frames) {
			...
			if (objLoc.isSequence) {
			    frameSrc = assetsLoc + book.assetLoc.img + objLoc.sequenceFolderName + "/" + objLoc.frameNames[f];
			} else {
			    frameSrc = assetsLoc +  book.assetLoc.img + "gifFrames/" + objLoc.fileName + "-frame-" + f + ".png";
			}
			...
			frame.onerror = function (e) {
			    console.log("Bug: " + "Problem loading gif/sequence frames with object <b>" + objLoc.name + "</b>.");
			    framesLoaded++;
			...
			console.log("Bug: " + "Image <b>" + objLoc.src + "</b> did not load");
			/*
		...
		if (isFirefox) {
		    src = assetsLoc + book.assetLoc.aud + name + ".ogg";
		} else {
		    src = assetsLoc + book.assetLoc.aud + name + ".wav";
		}
		aud.id = name;
		aud.errorNum = 0;
		...
		aud.onerror = function () {
		    if (aud.errorNum == 0) {
			aud.errorNum++;
			aud.src = book.assetLoc.aud + name + '.mp3';
		    } else {
                        aud.errored = true;
			console.log("Bug: " + "Audio <b>" + aud.src + "</b> did not load");
			nextBuffer(bufNum);
		    }
		}
		aud.src = src;
		...
		if (!curObj.created) {
			...
			console.log("Bug: " + "Video <b>" + curObj.name + "</b> did not load, check console for details");
	    ...
	    console.log("Bug: " + 'Unknown buffer type <b>' + type + '</b>, asset skipped.');
	    nextBuffer(bufNum);
	}
    }
}
